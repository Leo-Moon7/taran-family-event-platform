(function () {
  ...
  function getDistricts(province) { return data.find((item) => item.province === province)?.districts || []; }
  function fill(select, values, firstLabel, selected = "") {
    select.replaceChildren();
    const first = document.createElement("option"); first.value = ""; first.textContent = firstLabel; select.append(first);
  ...
  function setupSelects(provinceSelect, districtSelect, options = {}) {
    fill(provinceSelect, data.map((item) => item.province), options.provinceLabel || "시·도를 선택해 주세요", options.province || "서울특별시");
    const updateDistricts = (selected = "") => {
      const districts = getDistricts(provinceSelect.value);
      fill(districtSelect, districts, districts.length ? "시·군·구 선택 (선택)" : "하위 지역 없음", selected);
      districtSelect.disabled = !districts.length;
    };
    provinceSelect.addEventListener("change", () => updateDistricts(""));
    updateDistricts(options.district || "");
    return updateDistricts;
  }

  window.SonpumRegions = Object.freeze({ all: data.map((item) => item.province), data, defaultProvince: "서울특별시", getDistricts, setupSelects });
})();
