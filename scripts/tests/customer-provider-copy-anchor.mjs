...
check("seven provider projection and categories", () => {
  assert.equal(profiles.length, 7, "customer-ready profiles must be exactly seven");
  assert.equal(Array.from(profiles, ({ id }) => id).join("|"), expectedIds.join("|"), "the original five and only the two approved additions must remain in order");
  assert.equal(placeProfiles.length, 4, "place/meal profiles must be exactly four");
  assert.equal(photoProfiles.length, 3, "snap/video profiles must be exactly three");
  assert.equal(priceProfiles.length, 2, "numeric price profiles must remain two");
...
check("new provider identity, contacts, evidence, and official hosts", () => {
  for (const [id, expected] of nextProviderContracts) {
    ...
    assert.equal(profile.products.length, 0, `${id}: unverified branch price must stay hidden`);
    ...
    assert.ok(profile.contact.officialLinks.length >= 2, `${id}: official detail links required`);
...
check("static and runtime seven-provider directory contract", () => {
  assert.match(sources.venuesHtml, /id="directory-heading-count">서울 돌잔치 업체 7곳</u, "static heading count must be seven");
  assert.match(sources.venuesHtml, /data-category-tab="all">전체 <span>7<\/span>/u, "static total tab count must be seven");
  assert.match(sources.venuesHtml, /data-category-tab="돌잔치 장소·식사">장소·식사 <span>4<\/span>/u, "static place tab count must be four");
...
check("reference menu price and contact layout", () => {
  assert.match(productRenderer, /title\.textContent = "참고 메뉴 가격";/u, "price-ready details must label prices as reference menu prices");
...
if (failures.length) {
  console.error(`FAIL QA-055 customer provider expansion/copy/anchor (${failures.length})`);
  failures.forEach((failure, index) => console.error(`${index + 1}. ${failure}`));
  process.exitCode = 1;
} else {
  console.log("PASS QA-055 customer provider expansion/copy/anchor");
  console.log("profiles=7 categories=7/4/3 price=2 inquiry=5 images=0 travel=0 capabilities=0");
