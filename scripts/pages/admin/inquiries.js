(function () {
  ...
  function daysSince(value) {
    ...
    return time ? (Date.now() - time) / 86400000 : 0;
  }

  function buildOnlineRows({ claims, registrations, recipients, providers }) {
    ...
    const unansweredRows = recipients
      .filter((item) => item.status === "sent" && daysSince(item.sent_at) >= 2)
      .map((item) => ({
        id: item.id,
        type: "unanswered",
        createdAt: item.sent_at,
        label: "장기 미응답",
        target: providerNames.get(String(item.provider_id)) || item.provider_id,
        reason: `${Math.floor(daysSince(item.sent_at))}일 동안 문의 미열람`,
        status: "확인 필요",
        href: `../provider.html?id=${encodeURIComponent(item.provider_id)}`
      }));
    return [...claimRows, ...registrationRows, ...deliveryRows, ...unansweredRows]
  ...
  async function init() {
    ...
    if (access.mode === "online") {
      const [claims, registrations, recipients, providers] = await Promise.all([
        window.TaranAdminData.list("providerClaims", { order: "created_at.desc", limit: 300 }),
        window.TaranAdminData.list("providerRegistrations", { order: "created_at.desc", limit: 300 }),
        window.TaranAdminData.list("inquiryRecipients", { order: "sent_at.desc", limit: 500 }),
        window.TaranAdminData.list("providers", { select: "id,data", limit: 5000 })
      ]);
      rows = buildOnlineRows({ claims, registrations, recipients, providers });
    } else {
