...
기존 Supabase 프로젝트를 이전 버전 스키마로 이미 운영 중이라면 전체 스키마를 다시 붙여넣는 대신 `migrations/002_security_hardening.sql`을 실행합니다. 업체 계정의 전체 업체 접근을 차단하고 커뮤니티 글·댓글을 검수 대기 상태로 고정하는 보안 업데이트입니다.
