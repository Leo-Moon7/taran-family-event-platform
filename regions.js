(function () {
  ...
  const provinceAliases = {
    ...
    세종: "세종특별자치시", 경기: "경기도", 강원: "강원특별자치도", 충북: "충청북도", 충남: "충청남도", 전북: "전북특별자치도", 전남: "전라남도",
  ...
  window.taranResolveRegion = function (value) {
    ...
    const exactProvince = window.taranRegionData.find((entry) => entry.province === raw);
    if (exactProvince) return { province: exactProvince.province, district: "" };
    const alias = Object.entries(provinceAliases).find(([shortName]) => raw === shortName || raw.startsWith(`${shortName} `));
    const province = alias ? alias[1] : window.taranRegionData.find((entry) => raw.includes(entry.province))?.province || "";
    const candidates = province ? window.taranRegionData.filter((entry) => entry.province === province) : window.taranRegionData;
    for (const entry of candidates) {
      const district = entry.districts.find((name) => raw.includes(name));
      if (district) return { province: entry.province, district };
    }
    return { province: province || raw, district: "" };
  };

  window.taranRegionLabel = function (value) {
    const resolved = window.taranResolveRegion(value);
    return [resolved.province, resolved.district].filter(Boolean).join(" ") || "지역 정보 없음";
  };

  window.taranSetupRegionSelects = function (provinceSelect, districtSelect) {
    provinceSelect.replaceChildren();
    const allProvinces = document.createElement("option");
    allProvinces.value = "all";
    allProvinces.textContent = "전체 시·도";
    provinceSelect.append(allProvinces);
    ...
    const fillDistricts = () => {
      ...
      const allDistricts = document.createElement("option");
      allDistricts.value = "all";
      allDistricts.textContent = "전체 시·군·구";
      districtSelect.append(allDistricts);
      ...
      districtSelect.disabled = !entry?.districts.length;
    };
