...
## Revision 1 · 클립보드 제한 환경 보완

- 미리보기 상호작용 재검수에서 목록 필터는 장소·식사 5곳, 스냅·영상 3곳, 가격 정보 3곳, `오크우드` 검색 1곳으로 일치했다.
- 브라우저가 Clipboard API 권한을 거부할 때 임시 textarea와 `document.execCommand("copy")`를 사용하는 대체 복사 경로를 추가했다.
- 대체 필드는 복사 직후 제거하며, 두 방식이 모두 실패할 때만 기존 오류 안내를 표시한다.
- 구문 검사, 8곳 프로필 계약, QA-059 UI 계약, validate, build, test:dist를 다시 실행해 모두 통과했다.
- Netlify 재배포 미리보기에서 Clipboard API가 제한된 상태에서도 `문의 내용이 복사되었습니다` 성공 안내를 확인했다.
